...
def create_tree_plot(gene_data_file: str, pathway_hierarchy_file: str, output_image: str) -> None:
    ...
    hierarchy_tree = {
        'root': {
            'node': root,
            'children': {},
            'angle_range': (0, 2 * np.pi)  # 根节点可以使用整个圆
        }
    }
    ...
    # Draw edges with improved style
    edge_lines = []
    edge_colors = []
                    ...
                    if v_depth < 3:  # 对于非最外层的连接
                        ...
                        for i in range(len(curve_points)-1):
                            ...
                            if v_depth == 2:
                                ...
                                alpha = max(0.15, 0.4 - v_depth * 0.1)
    ...
    # Set color mapping - use academic style colors
    level_colors = {
        1: ['#4A90E2', '#8E44AD', '#2ECC71', '#34495E', '#E67E22'],  # Dark academic colors
        2: ['#7FB3D5', '#BB8FCE', '#82E0AA', '#85929E', '#F5B041']   # Corresponding light colors
    }
    
    # Pre-calculate color gradients for better performance
    def create_color_gradient(up_ratio: float, down_ratio: float, 
                            none_ratio: float, num_points: int = 50) -> np.ndarray:
        """
        Create a color gradient array based on gene regulation ratios.
        ...
        down_ratio : float
            Ratio of down-regulated genes
        none_ratio : float
            Ratio of unchanged genes
        num_points : int
        ...
        --------
        np.ndarray
            Array of RGBA colors for the gradient
        """
    ...
    for node, (x, y) in pos.items():
        try:
            ...
            if depth == 3:  # Pathway level
                ...
                # Set parameters
                inner_radius = 1.45  # Inner radius
                outer_radius = 2.0   # Outer radius
                ...
                # Get pre-calculated color gradient
            ...
            elif depth in [1, 2]:  # Second and third layers
                # Calculate node size
                num_neighbors = len(list(nx.neighbors(G, node)))
                base_size = 0.15 if depth == 1 else 0.08
                node_size = base_size + num_neighbors * 0.01
                
                # Set transparency
                node_alpha = 0.9 if depth == 1 else 0.4
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Create a KEGG pathway tree visualization')
